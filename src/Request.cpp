...
UINT Stm32NetXHttpWebClient::Request::initialize(UINT method, CHAR *resource, UINT resource_length, CHAR *host,
    ...
    client->log(Stm32ItmLogger::LoggerInterface::Severity::INFORMATIONAL)
            ->println("Stm32NetXHttpWebClient::Request::initialize()");


    if (!client->flags.isSet(Client::IS_CONNECTED)) {
    ...
    const auto ret = nx_web_http_client_request_initialize_extended(client,
                                                                    method,
                                                                    resource,
                                                                    resource_length,
                                                                    host,
    ...
    if (ret != NX_SUCCESS) {
        client->log(Stm32ItmLogger::LoggerInterface::Severity::ERROR)
                ->printf(
                    "Stm32NetXHttpWebClient::Request[%s]: nx_web_http_client_request_initialize_extended() = 0x%02x\r\n",
                    client->getName(), ret);
    }
    client->flags.set(Client::IS_INITIALIZED);
    return ret;
}
...
UINT Stm32NetXHttpWebClient::Request::send(UINT wait_option) {
    ...
    const auto ret = nx_web_http_client_request_send(client, wait_option);
...
UINT Stm32NetXHttpWebClient::Request::headerAdd(CHAR *field_name, UINT name_length, CHAR *field_value,
    ...
    const auto ret = nx_web_http_client_request_header_add(client, field_name, name_length, field_value, value_length,
                                                           wait_option);
...
UINT Stm32NetXHttpWebClient::Request::packetAllocate(NX_PACKET **packet_ptr, ULONG wait_option) {
    ...
    const auto ret = nx_web_http_client_request_packet_allocate(client, packet_ptr, wait_option);
...
UINT Stm32NetXHttpWebClient::Request::packetSend(NX_PACKET *packet_ptr, UINT more_date, ULONG wait_option) {
    ...
    const auto ret = nx_web_http_client_request_packet_send(client, packet_ptr, 0, wait_option);
